  ...
  useEffect(() => {
    const fetchEnquiries = async () => {
      try {
        ...
        const response = await axios.get('https://zeal-tourisam-api.vercel.app/api/globalvisa/VisagetEnquirys');
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {enquiries.map((enquiry) => (
          ...
          >
            ...
            <p className="text-gray-500">{enquiry.mobileNumber}</p>
            <p className="text-gray-600 font-medium mt-2">{enquiry.visaCategory}</p>
            <p className="text-gray-600">
              Date of Travel:{' '}
              <span className="text-gray-800 font-semibold">
                {new Date(enquiry.dateOfTravel).toLocaleDateString()}
              </span>
            </p>
